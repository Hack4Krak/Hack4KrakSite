//! `SeaORM` Entity, @generated by sea-orm-codegen 1.1.17

use super::sea_orm_active_enums::TeamStatus;
use sea_orm::entity::prelude::*;
use serde::{Deserialize, Serialize};

#[derive(
    Clone,
    Debug,
    PartialEq,
    DeriveEntityModel,
    Eq,
    Serialize,
    Deserialize,
    hack4krak_macros :: DeriveUpdatableModel,
    Default,
)]
#[sea_orm(table_name = "teams")]
#[hack4krak_macros::to_schema_from_file_name]
pub struct Model {
    #[sea_orm(unique)]
    pub name: String,
    pub created_at: DateTime,
    #[sea_orm(primary_key, auto_increment = false)]
    pub id: Uuid,
    pub confirmation_code: Option<Uuid>,
    pub status: TeamStatus,
    pub color: String,
    pub organization: Option<String>,
}

#[derive(Copy, Clone, Debug, EnumIter, DeriveRelation)]
pub enum Relation {
    #[sea_orm(has_many = "super::external_team_invitation::Entity")]
    ExternalTeamInvitation,
    #[sea_orm(has_many = "super::flag_capture::Entity")]
    FlagCapture,
    #[sea_orm(has_many = "super::team_invites::Entity")]
    TeamInvites,
    #[sea_orm(has_many = "super::users::Entity")]
    Users,
}

impl Related<super::external_team_invitation::Entity> for Entity {
    fn to() -> RelationDef {
        Relation::ExternalTeamInvitation.def()
    }
}

impl Related<super::flag_capture::Entity> for Entity {
    fn to() -> RelationDef {
        Relation::FlagCapture.def()
    }
}

impl Related<super::team_invites::Entity> for Entity {
    fn to() -> RelationDef {
        Relation::TeamInvites.def()
    }
}

impl Related<super::users::Entity> for Entity {
    fn to() -> RelationDef {
        Relation::Users.def()
    }
}

impl ActiveModelBehavior for ActiveModel {}
