...
async fn participate_success_sends_registration_confirmation_email() {
    ...
    let identification_code = smtp_client.find_uuid_in_first_email().await;
    assert_eq!(identification_code, user.identification_code.to_string());
}

#[actix_web::test]
async fn get_participation_returns_submitted_registration() {
    let test_database = TestDatabase::new().await;
    let user = test_database.with_default_user().await;

    let app = TestApp::default()
        .with_database(test_database)
        .with_smtp_client(MockSmtpClient::default())
        .build_app()
        .await;
