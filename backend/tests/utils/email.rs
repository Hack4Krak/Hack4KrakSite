...
async fn send_mail() {
    ...
    email
        .send(&Data::new(AppState::with_email_client(mail_client.smtp_client.clone())))
        .await
        .unwrap();
    ...
    assert_eq!(
        first_email.raw.from.as_str(),
        "user@domain.tld"
    );

    assert_eq!(
        first_email.raw.to[0].as_str(),
        "danger@domain.tld"
    );
    assert_eq!(
        first_email.raw.to[1].as_str(),
        "dziegiel@domain.tld"
    );
    assert_eq!(
        first_email.raw.to[2].as_str(),
        "borbert@domain.tld"
    );
}
