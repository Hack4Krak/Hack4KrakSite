...
function mountTimelineAt(now: string) {
  vi.setSystemTime(new Date(now))

  return mountSuspended(EventTimeline, {
    props: { stages },
  })
}
...
describe('eventTimeline', () => {
  beforeEach(() => {
    vi.useFakeTimers({ now: new Date('2026-05-23T07:00:00+02:00') })
  })
  ...
  it('groups stages by day and renders time ranges', async () => {
    const wrapper = await mountTimelineAt('2026-05-23T07:00:00+02:00')
  ...
  it('marks the latest active stage as current and dims past stages', async () => {
    const wrapper = await mountTimelineAt('2026-05-23T10:30:00+02:00')
  ...
  it('marks the final stage as past after it starts when it has no end date', async () => {
    const wrapper = await mountTimelineAt('2026-05-24T16:30:00+02:00')

    const finalStage = wrapper.find('[data-stage-type="event-end"]')

    expect(finalStage.attributes('data-stage-state')).toBe('past')
    expect(finalStage.classes()).not.toContain('bg-primary/10')
    expect(wrapper.find('[data-stage-state="current"]').exists()).toBe(false)
  })
})
